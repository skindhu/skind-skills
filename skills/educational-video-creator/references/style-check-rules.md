...
字幕定位检查:
- 字幕组件（SubtitleSequence / Subtitle）的 `bottom` 值应在 **60-120** 范围内
- `bottom < 60`: 🔴严重（超出安全区底边，字幕可能被裁切）
- `bottom > 120`: 🟡重要（字幕偏高，可能与场景内容重叠，标准值: `bottom: 60`）

**修复策略**: 元素底部侵入字幕区或与字幕间距 < 30px → 上移该元素（减小 top 值或增大 bottom 值）使其底部 ≤ 850（即与字幕区保持 ≥ 30px 间距）。字幕偏高 → 将 `bottom` 改为 80。
